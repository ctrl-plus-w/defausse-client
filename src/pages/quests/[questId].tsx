...
const Quest = () => {
  ...
  const onBlur = async (_event: FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    try {
      const res = await database.put<any, AxiosResponse<Quest>>(`/quests/${quest.id}`, { content });
  ...
  return (
    <div className='flex flex-col px-12 py-16'>
      <Breadcrumb items={[{ label: 'Quêtes' }, { label: `Quête(ID: ${quest!.id.toString()})` }]} />
      <h1 className='text-5xl font-medium text-gray-900 mb-4 mt-2'>Édition de quête</h1>
