...
import { ChangeEvent, Dispatch, FocusEvent, SetStateAction, useEffect, useState } from 'react';
...
const Mode = () => {
  ...
  const onChange = (func: Dispatch<SetStateAction<string>>) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    func(event.target.value);
  };

  const onBlur = async (_event: FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (!mode) return;

    // If any field has changed
    if (mode.description === description && mode.name === name && mode.summary === summary) return;

    try {
      const res = await database.put<any, AxiosResponse<Mode>>(`/modes/${mode.id}`, { name, description, summary });

      const { name: _name, description: _description, summary: _summary } = res.data;
  ...
  return (
    <div className='flex flex-col px-12 py-16'>
      <Breadcrumb items={[{ label: 'Modes' }, { label: `Mode(ID: ${mode!.id.toString()})` }]} />
      <h1 className='text-5xl font-medium text-gray-900 mb-4 mt-2'>{mode!.name}</h1>
      ...
      <form className='flex w-full gap-6'>
        <div className='flex flex-col flex-1 gap-6'>
          <Input name='name' label='Nom' value={name} onChange={onChange(setName)} onBlur={onBlur} />
          <TextArea name='summary' label='Résumé' value={summary} onChange={onChange(setSummary)} onBlur={onBlur} blurOnEnter={false} rows={5} />
        </div>
