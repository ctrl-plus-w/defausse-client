...
const LocationsDisplay = ({ locations, playerIntervalId, className }: IProps) => {
  ...
  const onBlurNewLocation = async (_event: FocusEvent<HTMLInputElement>) => {
    ...
    try {
      await database.post<any, AxiosResponse<Location>>(`/locations`, { name: newLocation }, { params: { playerIntervalId: playerIntervalId } });
