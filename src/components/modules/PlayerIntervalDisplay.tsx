...
const PlayerIntervalDisplay = ({ playerInterval }: IProps) => {
	...
	const [hasInterval, setHasInterval] = useState(
		playerInterval.min != playerInterval.max
	);
	...
	const updateValue = (
		...
		return (event: ChangeEvent<HTMLInputElement>) => {
			...
			if (
				(value !== '' && !isNum(value)) ||
				parseInt(value) > config.MAX_INTERVAL_VALUE
			)
				return;
	...
	const updateDatabaseValues = async () => {
		...
		if (
			parseInt(min) !== playerInterval.min &&
			parseInt(max) !== playerInterval.max
		) {
			if (max === '') {
		...
		try {
			await database.put<any, AxiosResponse<PlayerInterval>>(
	...
	return (
		<div className="relative flex flex-col">
			...
			<LocationsDisplay
				playerIntervalId={playerInterval.id}
				locations={playerInterval?.locations || []}
			/>
		</div>
	);
};
