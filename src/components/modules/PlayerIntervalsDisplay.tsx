...
const sortPlayerIntervals = (
	{ min: pi1Min, max: pi1Max }: PlayerInterval,
	{ min: pi2Min, max: pi2Max }: PlayerInterval
) => {
	if (pi1Min === pi2Min) {
...
const PlayerIntervalsDisplay = ({ className }: IProps) => {
	...
	const onBlur = async (_event: FocusEvent<HTMLInputElement>) => {
		...
		if (
			...
			isEmptyStr(min) ||
			!isNum(min) ||
			pMin > config.MAX_INTERVAL_VALUE
		)
			return;

		try {
			await database.post<any, AxiosResponse<PlayerInterval>>(
				'/playerIntervals',
				{ min: pMin, max: pMin },
	...
	return (
		<div
			className={clsx([
				'flex flex-col flex-1 gap-4 flex-1min-w-min',
				className,
			])}
		>
			...
			<div className="flex flex-col gap-6">
				{[...(script?.playerIntervals || [])]
					.sort(sortPlayerIntervals)
					.map(playerInterval => (
						<PlayerIntervalDisplay
							playerInterval={playerInterval}
							key={playerInterval.id}
						/>
					))}
