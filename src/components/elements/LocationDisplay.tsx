...
const LocationDisplay = ({ location, className }: IProps) => {
	...
	const onBlur = async () => {
		...
		try {
			await database.put<any, AxiosResponse<Location>>(
	...
	const onDelete = async (event: MouseEvent<HTMLButtonElement>) => {
		...
		try {
			await database.delete<any, AxiosResponse<Location>>(
				`/locations/${location.id}`
			);
	...
	return (
		<li
			className={clsx([
				'flex justify-between cursor-pointer hover:bg-gray-50 transition-colors duration-300',
				className,
			])}
		>
			<Link
				className="block pl-3 py-2 w-full"
				href={`/scripts/${script!.id}/locations/${location.id}`}
			>
				<InvisibleInput
					className={clsx([
			...
			<button
				className="px-3 py-2 text-gray-500 hover:text-pink-700 transition-colors duration-300"
				onClick={onDelete}
			>
				<XIcon />
			</button>
